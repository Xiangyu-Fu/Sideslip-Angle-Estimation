...
class SSAEstimation(object):
    def __init__(self):
        ...
        # intitialize parameters for EKF
        self.vel_y = 0
        self.P_k_minus_1 = np.array([[0.1, 0], [0, 0.1]])
    ...
    def EKFupdate(self, A_k, B_k, H_k, z_k_observation_vector, state_estimate_k_minus_1,
        ...
        # print('State Estimate After EKF={}'.format(state_estimate_k.T))
    ...
    def GetDM(self, processed_data, dk, v_y):
        """
        get parameters of dynamic model and process data
        :param processed_data: data getting from topic
        :param dk: time interval
        :param v_y: velocity in y direction
        :return: matrix of dynamic model, and some needed value in next step
        """
        # get data from topic
        ...
        B_d = np.array([[self.C_f / self.m_v],
                        [self.L_f * self.C_f / self.I_z]], dtype=np.float64)
        C_d = np.array([[(-self.C_f - self.C_r) / (self.m_v * vel_x),
                         -((self.L_f * self.C_f - self.L_r * self.C_r) / (self.m_v * vel_x))],
                        [0, 1]], dtype=np.float64)
        D_d = np.array([[self.C_f / self.m_v, 0]], dtype=np.float64).T

        # clamp values
        if 0.1 > steering_angle > -0.1:
            self.clamp_function()
        if 0.01 > yaw_rate > -0.01:
            self.clamp_function()
        if 0.001 > vel_x > -0.001:
            self.clamp_function()
        ...
        return A_d, B_d, C_d, D_d, y_d, steering_angle, x_d, vel_x, acc_y

    def EstCallback(self, processed_data):
        """
        side slip estimation callback function, the main function in this node
        :param processed_data: get processed data from data_process node
        :return: NA
        """
        # get sensor data
        ...
        time = rospy.get_time()
        dk = time - self.last_time
        self.last_time = time
        print('\n')

        # get parameters for model
        A_d, B_d, C_d, D_d, y_d, u_d, x_d, vel_x, acc_y = self.GetDM(data, dk, self.vel_y)

        if (acc_y < 5.0 and acc_y > -5.0):
            # EKF update
            optimal_state_estimate_k, P_k = self.EKFupdate(A_d, B_d, C_d, y_d, x_d, u_d, self.P_k_minus_1, dk, D_d=D_d)

            # exclude nan value
            NaN_sign = math.isnan(x_d[0, 0])
            if not NaN_sign:
                # update values
                ...
                # calculate Beta
                Beta = np.arctan(self.vel_y / vel_x)

                # clamp the range of Beta
                if Beta > 1.2 or Beta < -1.2:
                    Beta = 0
